...
本Skill是为了解决PPT工作中常见的PPT文件转发后，插入图表数据源表格断链导致编辑更改数据十分困难的问题，使用技术手段恢复PPT数据包中原始表格数据，新建本地表格重新链接回PPT图表，使得PPT中图表数据更新更加便利快捷

**注意**
1. 本Skill仍处于测试阶段，可能出现无法恢复以及数据错误的问题，在使用前注意备份数据
2. 本Skill运行阶段必须关闭您现有的所有PPT和Excel窗口，可能造成不便，我们会在后续迭代更新中尝试取消这一限制，但目前您必须谨记这一要求
3. 通过本Skill重新链接图表数据的PPT文档在再次转发时可能会再次断链，如果需要再转发时还能正常链接，需要您先向Agent发送以下指令 “请使用 repair-ppt-chart-data-links skill，按“跨电脑可迁移交付模式”处理这个 PPT：先扫描并让我确认图表，完成数据恢复、Excel 原生重连和 verify --office-required 全部通过后，再执行 portable-package，把完整数据包输出到【输出目录】，必须包含可迁移母版 PPT、data 工作簿、bundle-manifest、启动器和使用说明；不要覆盖原 PPT，不要在严格验收未通过时打包，也不要声称单个 PPT 可以脱离 Excel 在另一台电脑上保持链接。”
